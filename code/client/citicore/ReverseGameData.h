...
struct RageIOPad
{
	enum
	{
		TRIGGER_THRESHOLD = 64, // the original function uses a threshold at 0x141d8abb9, it is normally 64.

		BUTTON_LEFT_TRIGGER = (1 << 0), // 1 -- uses TRIGGER_THRESHOLD
		BUTTON_RIGHT_TRIGGER = (1 << 1), // 2 -- ^^^^
		BUTTON_LEFT_SHOULDER = (1 << 2), // 4
		BUTTON_RIGHT_SHOULDER = (1 << 3), // 8
		BUTTON_Y = (1 << 4), //0x10
		BUTTON_B = (1 << 5), //0x20
		BUTTON_A = (1 << 6), //0x40
		BUTTON_X = (1 << 7), //0x80
		BUTTON_BACK = (1 << 8), //0x100
		BUTTON_LEFT_THUMB = (1 << 9), //0x200
		BUTTON_RIGHT_THUMB = (1 << 10), //0x400
		BUTTON_START = (1 << 11), //0x800
		BUTTON_UP = (1 << 12), //0x1000
		BUTTON_RIGHT = (1 << 13), //0x2000
		BUTTON_DOWN = (1 << 14), //0x4000
...
struct ReverseGameData
{
	...
	void ResetGamePad()
	{
		...
		gamepad.stickValues = 0x80808080; // stick neutral = 127/255
		gamepad.isFresh = true; // set this in case someone wants to just assign the struct to the game's memory
	}
