...
NTSTATUS NTAPI LdrLoadDllStub(const wchar_t* fileName, uint32_t* flags, UNICODE_STRING* moduleName, HANDLE* handle)
{
	...
	if (moduleNameStr.find(L"MedalLoader.dll") != std::string::npos)
	{
		...
		taskDialogConfig.dwFlags = TDF_ENABLE_HYPERLINKS | TDF_EXPAND_FOOTER_AREA | TDF_CALLBACK_TIMER | TDF_USE_COMMAND_LINKS;
		taskDialogConfig.dwCommonButtons = 0;
		taskDialogConfig.cButtons = 3;
		taskDialogConfig.pButtons = buttons;
		taskDialogConfig.pszWindowTitle = L"Medal TV break games";
		taskDialogConfig.pszMainIcon = TD_INFORMATION_ICON;
		taskDialogConfig.pszMainInstruction = L"Get rid of Medal TV";
		taskDialogConfig.pszContent = L"Medal TV uses SharpDX. As of 29 Mar 2019, SharpDX is no longer being under development or maintenance. SharpDX has an issue that it crashes when it hooks DirectX 10.1 taking FiveM with it.";
		taskDialogConfig.pszExpandedInformation = L"SharpDX is broken.";
		taskDialogConfig.pfCallback = [](HWND hWnd, UINT type, WPARAM wParam, LPARAM lParam, LONG_PTR data)
		{
			if (type == TDN_BUTTON_CLICKED)
			{
				if (wParam == 42)
				{
					std::string MedalPath = getenv("LOCALAPPDATA");
					MedalPath += "\\Medal\\Update.exe";
					STARTUPINFOA si = { 0 };
					si.cb = sizeof(si);
				...
				else if (wParam == 43)
				{
					...
					while (hRes)
					{
						if (lstrcmp(pEntry.szExeFile, L"Medal.exe") == 0 || lstrcmp(pEntry.szExeFile, L"MedalInjector.exe") == 0)
						{
