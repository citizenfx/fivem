			...
			if (s_references.TryGetValue(reference, out var funcRef))
			{
				...
				if (result is Coroutine coroutine)
				{
					if (coroutine.IsCompleted)
					{
						...
						{
							return MsgPackSerializer.Serialize(new object[] { false, coroutine.Exception.ToString() });
						}

						return MsgPackSerializer.Serialize(new object[] { true, new[] { coroutine.GetResultNonThrowing() } });
					}
					else
					{
						...
						return MsgPackSerializer.Serialize(new object[] { true, new object[] { returnDictionary } });
					}
				}

				return MsgPackSerializer.Serialize(new object[] { true, new object[] { result } });
			}
			else
			{
