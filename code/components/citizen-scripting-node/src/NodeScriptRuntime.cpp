...
static InitFunction initFunction([]()
{
	...
	if (FX_FAILED(hr))
	{
		//NOTE: why exit in cases when the runtime is not used?
		//ExitProcess(0); 
		return;
	}
...
static std::pair<std::string, v8::FunctionCallback> g_citizenFunctions[] =
{
	...
	// natives
	{ "invokeNative", V8_InvokeNative<NodeScriptRuntime, StringHashGetter> },
	{ "invokeNativeByHash", V8_InvokeNative<NodeScriptRuntime, IntHashGetter> },
	...
	// metafields
	{ "pointerValueIntInitialized", V8_GetPointerField<V8MetaFields::PointerValueInt, NodeScriptRuntime> },
	{ "pointerValueFloatInitialized", V8_GetPointerField<V8MetaFields::PointerValueFloat, NodeScriptRuntime> },
...
result_t NodeScriptRuntime::CallRef(int32_t refIdx, char* argsSerialized, uint32_t argsLength, char** retvalSerialized, uint32_t* retvalLength)
{
	*retvalLength = 0;
...
const char* NodeScriptRuntime::AssignStringValue(const v8::Local<v8::Value>& value)
{
