...
std::optional<std::string> ResourceCache::GetCustomEntry(const std::string& key)
{
	// attempt a database get
	std::string value;

	auto fetchIt = m_customEntryCache.find(key);

	if (fetchIt != m_customEntryCache.end() && fetchIt->second)
	{
		return *fetchIt->second;
	}
	...
	{
#if _DEBUG
		FatalError("Failed to fetch custom ResourceCache entry: %s", status.ToString());
#else
		trace("Failed to fetch custom ResourceCache entry: %s\n", status.ToString());
#endif
	}

	m_customEntryCache[key] = { std::optional<std::string>{} };
	return {};
}

void ResourceCache::AddCustomEntry(const std::string& key, const std::string& value)
{
	leveldb::WriteOptions options;
	options.sync = true;

	m_indexDatabase->Put(options, key, leveldb::Slice(value.data(), value.size()));
	m_customEntryCache[key] = {};
}
