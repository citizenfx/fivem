...
// Wheel class
const int WheelCamberOffset = 0x008;
const int WheelCamberInvertedOffset = 0x010;
...
static HookFunction initFunction([]()
{
	...
	fx::ScriptEngine::RegisterNativeHandler("GET_VEHICLE_WHEEL_CAMBER", makeWheelFunction([](fx::ScriptContext& context, fwEntity* vehicle, uintptr_t wheelAddr)
	{
		context.SetResult<float>(*reinterpret_cast<float *>(wheelAddr + WheelCamberOffset));
	}));

	fx::ScriptEngine::RegisterNativeHandler("SET_VEHICLE_WHEEL_CAMBER", makeWheelFunction([](fx::ScriptContext& context, fwEntity* vehicle, uintptr_t wheelAddr)
	{
		*reinterpret_cast<float *>(wheelAddr + WheelCamberOffset) = context.GetArgument<float>(2);
		*reinterpret_cast<float *>(wheelAddr + WheelCamberInvertedOffset) = -context.GetArgument<float>(2);
	}));
});
