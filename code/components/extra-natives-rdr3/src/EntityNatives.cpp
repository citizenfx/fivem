...
static HookFunction hookFunction([]()
{
	fx::ScriptEngine::RegisterNativeHandler("GET_ENTITIES_IN_RADIUS", [](fx::ScriptContext& context)
	{
		...
		auto checkPos = DirectX::XMVectorSet(context.GetArgument<float>(0), context.GetArgument<float>(1), context.GetArgument<float>(2), 0.0f);
		...
		for (int i = 0; i < objectPool->GetSize(); i++)
		{
			...
			auto entityPos = DirectX::XMVectorSet(position.x, position.y, position.z, 0.0f);

			
			float distSq = DirectX::XMVectorGetX(DirectX::XMVector3LengthSq(entityPos - checkPos));
