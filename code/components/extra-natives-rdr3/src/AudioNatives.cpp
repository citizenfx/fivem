...
static bool g_DISABLE_DEATH_AUDIO_SCENE = false;
...
static HookFunction hookFunction([]()
{
		...
		// 0x48A61B5F, ambient sounds
		{
			...
			static struct : jitasm::Frontend
			{
				...
				virtual void InternalMain() override
				{
					...
					mov(rax, (uintptr_t)&g_DISABLE_DEATH_AUDIO_SCENE);
	...
	fx::ScriptEngine::RegisterNativeHandler("SET_DISABLE_DEATH_AUDIO_SCENE", [](fx::ScriptContext& context) 
	{
		bool value = context.GetArgument<bool>(0);

		g_DISABLE_DEATH_AUDIO_SCENE = value;
		...
		if (FX_SUCCEEDED(fx::GetCurrentScriptRuntime(&runtime)))
		{
			...
			{
				g_DISABLE_DEATH_AUDIO_SCENE = false;
			});
		}
	});
});