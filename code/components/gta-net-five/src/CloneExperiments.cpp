...
static void NetworkEventComponentControlBase_Write(CNetworkEventComponentControlBase* self, rage::datBitBuffer* buffer)
{
	...
	buffer->WriteUns(self->m_vehicleId, kObjectIdLength);
	buffer->WriteUns(self->m_entityId, kObjectIdLength);
	buffer->WriteUns(self->unk_C, 6);
	buffer->WriteBit(self->unk_D);

	trace("NetworkEventComponentControlBaseWrite | [vehicleId: %d] [entityId: %d] [unk_C %d] [unk_D %d] \n", self->m_vehicleId, self->m_entityId, self->unk_C, self->unk_D);
}
...
static void NetworkEventVehComponentControl_Write(CNetworkEventVehComponentControl* self, rage::datBitBuffer* buffer)
{
	...
	static const int kObjectIdLength = icgi->OneSyncBigIdEnabled ? 16 : 13;
	trace("NetworkEventVehComponentControlWrite | [occupantObjectID: %d] [unk_d: %d] [unk_1a: %d]\n", self->m_occupantObjectId, self->unk_D, self->unk_1A);

	buffer->WriteBit(self->unk_1A);
	if (!self->unk_D && self->unk_1A)
	{
		buffer->WriteUns(self->m_occupantObjectId, kObjectIdLength);
		return;
	}

	trace("failed to write object id \n");
}
...
static void NetworkEventVehComponentControl_Reply(CNetworkEventVehComponentControl* self, rage::datBitBuffer* buffer)
{
	...
	if (!self->unk_E)
	{
		trace("NetworkEventVehComponentControl_WriteInner unk_e false bailing\n");
		return;
	}
