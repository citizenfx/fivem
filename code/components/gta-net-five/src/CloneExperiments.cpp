...
// 1103
static rage::netPlayerMgrBase* g_playerMgr = (rage::netPlayerMgrBase*)0x14273D920;
...
namespace sync
{
	void TempHackMakePhysicalPlayer(uint16_t clientId, int idx = -1)
	{
		...
		((void(*)(void*))0x1410407E8)(nonphys); // ctor

		void* phys = calloc(256, 1);
		((void(*)(void*))0x14103ECA8)(phys);
...
void HandleCliehtDrop(const NetLibraryClientInfo& info)
{
	if (info.netId != g_netLibrary->GetServerNetID())
	{
		...
		// 1103
		uint16_t objectId;
		auto ped = ((void*(*)(void*, uint16_t*, CNetGamePlayer*))0x140FC1D08)(nullptr, &objectId, player);
		...
		if (ped)
		{
			...
			((void(*)(void*, uint16_t, CNetGamePlayer*))0x140FA9BB4)(ped, objectId, player);
...
static CNetGamePlayer* AllocateNetPlayer()
{
	...
	// 1103
	return ((CNetGamePlayer*(*)(void*))0x14103EB28)(plr);
}
...
static HookFunction hookFunction([]()
{
	...
	// for netblendervehicle
	//uintptr_t* vmt = (uintptr_t*)0x14196B3C8;

	//for (int i = 0; i < 40; i++)

	// for cnetworkgayermanager
	//uintptr_t* vmt = (uintptr_t*)0x141959B08;

	//for (int i = 0; i < 21; i++)

	{
		uintptr_t* vmt = (uintptr_t*)0x141963898;
		...
		{
			auto stub = new ReturnedCallStub(i * 8, vmt[i]);
			vmt[i] = (uintptr_t)stub->GetCode();
		}
	}

	{
		uintptr_t* vmt = (uintptr_t*)0x141959968;
	...
	/*hook::put(0x1419487D8, NetLogStub_DoLog);
	hook::put(0x1419487C0, NetLogStub_DoTrace);
	hook::jump(0x140A07E08, NetLogStub_DoLog);*/
	...
	hook::jump(0x141046E34, AllocateNetPlayer);

	hook::jump(hook::get_pattern("8A 41 49 3C FF 74 17 3C 20 73 13 0F B6 C8"), netObject__GetOwnerNetPlayer);
...

	{
		...
		{
			hook::jump(match.get(i).get<void>(0), GetPlayerByIndex);
		}
	}

	hook::jump(hook::get_pattern("48 83 EC 28 33 C0 38 05 ? ? ? ? 74 0A"), GetPlayerByIndexNet);

	hook::jump(hook::get_pattern("75 07 85 C9 0F 94 C3 EB", -0x19), IsNetworkPlayerActive);
	hook::jump(hook::get_pattern("75 07 85 C9 0F 94 C0 EB", -0x13), IsNetworkPlayerActive); // connected
...
static void ExecuteNetGameEvent(void* eventMgr, rage::netGameEvent* ev, rage::netBuffer* buffer, CNetGamePlayer* player, CNetGamePlayer* unkConn, uint16_t evH, uint32_t, uint32_t)
{
	//trace("executing a %s\n", ev->GetName());
...
static HookFunction hookFunctionEv([]()
{
...

	{
		...
		hook::jump(hook::get_call(location + 7), EventMgr_AddEvent);
	}

	hook::jump(hook::get_pattern("48 8B DA 48 8B F1 41 81 FF 00", -0x2A), ExecuteNetGameEvent);

	// can cause crashes due to high player indices, default event sending
	hook::jump(hook::get_pattern("48 83 EC 30 80 7A 2D FF 4C 8B D2", -0xC), Return1);
});
