...
static uint64_t(*g_origSyncedVehicle_GetMaxBits)(void*);
...
static uint64_t(*g_origSyncedUnk4720_GetMaxBits)(void*);
static uint64_t(*g_origSyncedSecondaryAnimation_GetMaxBits)(void*);
...
static uint64_t SyncedVehicle_GetMaxBits(void* self)
{
	return OverrideMaxBits(g_origSyncedVehicle_GetMaxBits, self);
}
...
{
	auto value = g_origSyncedTargetAI_GetMaxBits(self);
	// + 3 for target ObjectId
	// + 10 for playerIndex (6 -> 16 bit)
	return ExtendMaxBits<3 + 10>(g_origSyncedTargetAI_GetMaxBits, self);
}
...
static uint64_t SyncedSecondaryAnimation_GetMaxBits(void* self)
{
	return ExtendMaxBits(g_origSyncedSecondaryAnimation_GetMaxBits, self);
}

static uint64_t GetSyncedDynamicMaxBits(void* self)
{
	auto val = g_origSyncedUnk4720_GetMaxBits(self);
	return (val == 13 && icgi->OneSyncBigIdEnabled) ? 16 : val;
}
...
static HookFunction hookSyncedExtensions([]()
{
	...
	// CSyncedObjectId
	{
		...
		hook::put(&syncedObjectIdVtable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedVehicle
	{
		auto syncedUnkVTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 49 89 00 4D 89 50", 3));
		g_origSyncedVehicle_GetMaxBits = (decltype(g_origSyncedVehicle_GetMaxBits))syncedUnkVTable[kSyncedVarGetMaxBitsIndex];
		hook::put(&syncedUnkVTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)SyncedVehicle_GetMaxBits);
	}

	// CSyncedUnk5180 / CSyncedUnk8B70
	{
	...
	// CSyncedTarget<CAITarget>
	{
		...
		hook::put(&syncedAITargetVTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}
	...
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 0D ? ? ? ? 48 89 08 4C 89 48", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnk4720
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 48 89 83 ? ? ? ? B9 ? ? ? ? 44 88 B3 ? ? ? ? 81 A3", 3));
		g_origSyncedUnk4720_GetMaxBits = (decltype(g_origSyncedUnk4720_GetMaxBits))syncedTable[kSyncedVarGetMaxBitsIndex];
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	// CSyncedAnimationSecondary
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 0D ? ? ? ? 48 89 08 44 89 48 ? 83 48", 3));
		g_origSyncedSecondaryAnimation_GetMaxBits = (decltype(g_origSyncedSecondaryAnimation_GetMaxBits))syncedTable[kSyncedVarGetMaxBitsIndex];

		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)SyncedSecondaryAnimation_GetMaxBits);
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnkC420 - GetMaxBits is the same as CSyncedUnk4720
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 44 89 56", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	// CSyncedUnk3320 - GetMaxBits is the same as CSyncedUnk4720
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("4C 8D 15 ? ? ? ? 48 89 97", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	// CSyncedUnk4F28 - GetMaxBits is the same as CSyncedUnk4720
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 48 89 83 ? ? ? ? 0F 57 C0 40 88 B3 ? ? ? ? B9", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	// CSyncedUnkC940 - GetMaxBits is the same as CSyncedUnk4720
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 89 9F ? ? ? ? 48 89 87 ? ? ? ? 65 48 8B 04 25 ? ? ? ? 66 89 9F", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	// CSyncedUnkD8E8 - GetMaxBits is the same as CSyncedUnk4720
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 89 AB ? ? ? ? 48 89 83 ? ? ? ? 65 48 8B 04 25 ? ? ? ? 48 89 AB", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	// CSyncedUnk8948 - GetMaxBits is the same as CSyncedUnk4720
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 89 59", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	// CSyncedUnkD2F8
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 48 89 02 89 72 ? 85 F6 75 ? 44 38 3D ? ? ? ? 75 ? 48 8B CA E8 ? ? ? ? 48 8D 8B", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	// CSyncedUnkF190
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 89 69 ? 48 89 01 89 71", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	// CSyncedUnkCA48
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 35 ? ? ? ? 89 69", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	// CSyncedUnk7B88
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 45 89 90", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	//CSyncedUnk6B80
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 2D ? ? ? ? 4C 89 42", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	// CSyncedHeading (can sometimes be 13 bits, which then gets read/written as 16 bits from length hack)
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 4D 89 56", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	//CSyncedUnkE218
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("4C 8D 2D ? ? ? ? 8B 47", 3));
		hook::put(&syncedTable[kSyncedVarGetMaxBitsIndex], (uintptr_t)GetSyncedDynamicMaxBits);
	}

	//CSyncedUnk33F8
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 48 89 87 ? ? ? ? 48 B9 ? ? ? ? ? ? ? ? 8B 05", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnk10F0
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 44 89 A7 ? ? ? ? 48 89 87 ? ? ? ? 48 8B D5", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnk1340
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("45 33 E4 41 BD ? ? ? ? 48 89 8F", 96), 3, 7);
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnk7508
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 48 89 03 48 8B CB 8B 02", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnk4820
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 44 89 BF ? ? ? ? 48 8D 8F ? ? ? ? 48 89 87 ? ? ? ? E8", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedInt64
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("4C 8D 3D ? ? ? ? 44 89 AB", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnk2F48
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 48 8D 4B ? 48 89 03 33 D2 E8 ? ? ? ? 48 8D 4B ? 33 D2 E8 ? ? ? ? 83 63", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnkCF48
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 0D ? ? ? ? 89 6E", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnk5DC8
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 0D ? ? ? ? 48 89 08 44 89 48 ? 44 88 48", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}
	
	// CSyncedUnk6468
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 0D ? ? ? ? 44 89 48 ? 48 89 08 83 C9", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnk6530
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 4C 89 76 ? 48 89 06 48 8D 93", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnkCE80
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 44 89 49 ? 48 89 01 48 8D 51", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}

	// CSyncedUnkCDB8
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 05 ? ? ? ? 44 89 49 ? 48 89 01 41 B8", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}
	...
	// CSyncedUnk8D58
	{
		auto syncedTable = hook::get_address<uintptr_t*>(hook::get_pattern("48 8D 0D ? ? ? ? 48 89 08 83 C9 ? 66 89 48", 3));
		hook::put(&syncedTable[kSyncedVarUsesIdMappingsIndex], (uintptr_t)ReturnUsesIdMappings<false, true>);
	}
