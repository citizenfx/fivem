...
namespace fx
{
	namespace ServerDecorators
	{
		...
		const fwRefContainer<ServerImpl>& WithOutOfBandImpl(const fwRefContainer<ServerImpl>& server)
		{
			...
			static std::map<std::string, std::function<void(const fwRefContainer<ServerImpl>& server, const net::PeerAddress& from, const std::string_view& data)>, std::less<>> processors;
