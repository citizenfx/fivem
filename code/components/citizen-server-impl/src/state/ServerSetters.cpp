...
{
	auto tree = std::static_pointer_cast<sync::CTrainSyncTree>(MakeVehicle<sync::CTrainSyncTree>(model, posX, posY, posZ, heading, resourceHash));
...
static InitFunction initFunction([]()
{
	fx::ServerInstanceBase::OnServerCreate.Connect([](fx::ServerInstanceBase* ref)
	{
		...
		fx::ScriptEngine::RegisterNativeHandler("CREATE_TRAIN", [=](fx::ScriptContext& ctx)
		{
			...
			if (trainConfigIndex > (trainConfigs->GetData().empty() ? (uint32_t)(Is3095() ? 27 : (Is2802() ? 26 : (Is2372() ? 25 : 24))) : trainConfigs->GetData().size()))
			{
