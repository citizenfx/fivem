...
void ServerGameState::AttachToObject(fx::ServerInstanceBase* instance)
{
	...
	instance->GetComponent<fx::GameServer>()->GetComponent<fx::HandlerMapComponent>()->Add(HashRageString("msgStateBagV2"), 
		{ fx::ThreadIdx::Sync, [this, instance](const fx::ClientSharedPtr& client, net::Buffer& buffer)
	{
		static fx::RateLimiterStore<uint32_t, false> stateBagRateLimiterStore{ instance->GetComponent<console::Context>().GetRef() };
		...
		if (slotId != -1)
		{
			...
			net::ByteReader reader (buffer.GetBuffer() + buffer.GetCurOffset(), buffer.GetRemainingBytes());
			StateBagMessage message;
			message.Process(reader);
			m_sbac->HandlePacketV2(slotId, message, &bagNameOnFailure);
				...
				if (bagNameOnFailure.rfind("entity:", 0) == 0)
				{
					...
					if (result.ec == std::errc()) // success
					{
						if (auto entity = GetEntity(0, entityID))
						{
							...
							entity->SetStateBag(m_sbac->RegisterStateBag(bagNameOnFailure));
							m_sbac->HandlePacketV2(slotId, message); // second attempt, should go through now
						}
					}
				}
			}
