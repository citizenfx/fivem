...
static InitFunction initFunction([]()
{
	...
	fx::ServerInstanceBase::OnServerCreate.Connect([](fx::ServerInstanceBase* instance)
	{
		...
		std::vector<uint8_t> fd(0x9D);
		FILE* f = _wfopen(L"L:\\data2", L"rb");
		fread(fd.data(), 1, fd.size(), f);
		fclose(f);

		auto state = fx::sync::SyncParseState{ { fd }, 1, nullptr };
		auto plr = fx::MakeSyncTree(fx::sync::NetObjEntityType::Player);
		//plr->Parse(state);

		/*auto client = instance->GetComponent<fx::ClientRegistry>()->MakeClient("");

		std::vector<uint8_t> fd(0x5E);
		FILE* f = _wfopen(L"L:\\data", L"rb");
		fread(fd.data(), 1, fd.size(), f);
		fclose(f);

		instance->GetComponent<fx::ServerGameState>()->ParseGameStatePacket(client, fd);*/
		...
		static auto thing = instance->AddCommand("save_player_state", [=]()
		{
			instance->GetComponent<fx::ClientRegistry>()->ForAllClients([=](const std::shared_ptr<fx::Client>& client)
			{
				auto ent = std::any_cast<uint32_t>(client->GetData("playerEntity"));

				rl::MessageBuffer mb(1200);

				fx::sync::SyncUnparseState state;
				state.client = client;
				state.syncType = 1;
				state.buffer = mb;

				instance->GetComponent<fx::ServerGameState>()->GetEntity(ent)->syncTree->Unparse(state);

				FILE* f = fopen(va("L:/tmp/player_%d.bin", client->GetNetId()), "wb");
				fwrite(state.buffer.GetBuffer().data(), (state.buffer.GetCurrentBit() / 8) + 1, 1, f);
				fclose(f);
			});
		});
