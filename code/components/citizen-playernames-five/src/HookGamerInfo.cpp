...
int HGI_SprintfToBuffer(char* buffer, size_t length, const char* format, const char* name)
{
...
{
	// gamer tag creation
	hook::call(hook::get_pattern("48 8D 8F ? ? 00 00 C6 87 B0 00 00 00 01 E8", 14), HGI_SprintfToBuffer);

	// gamer tag name setting
	hook::call(hook::get_pattern("48 8D 8D ? 02 00 00 4C 8B CE", 15), HGI_SprintfToBuffer);
	...
	auto matches = hook::pattern("75 ? ? 10 87 00 00");
	...
	matches = hook::pattern("8D ? 80 85 00 00");
	...
	matches = hook::pattern("74 ? ? ? ? 98 86 00 00");
	...
	matches = hook::pattern("48 8D 87 ? ? ? ? 80 78 ? ? 75 ? 80 38 ? 75 ? 80 78 ? ? 75 ? 80 78 ? ? 74 ? 48");
	...
	for (int i = 0; i < matches.size(); i++)
	{
		...
		hook::put<uint32_t>(loc + 3, 0x40 + sizeof(CGamerTag) * GAMER_TAG_FOR_NET_PLAYER_LIMIT + 0xC7);
	}
	...
	matches = hook::pattern("83 FB ? 77 ? 48 69 DB"); // GAMER_INFO::gamerTagIsCrewConfigSet // GAMER_INFO::gamerTagSetCrew
