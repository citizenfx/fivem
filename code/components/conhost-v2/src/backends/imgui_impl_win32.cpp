...
#ifndef IS_FXSERVER
	if (mouse_viewport_id == ImGui::GetMainViewport()->ID)
	{
    ...
    if (HWND focused_hwnd = ::GetForegroundWindow())
    {
        ...
        if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
        {
            ...
            if (ImGui::FindViewportByPlatformHandle((void*)focused_hwnd) != NULL)
                io.AddMousePosEvent((float)mouse_screen_pos.x, (float)mouse_screen_pos.y);
        }
        else
        {
            ...
            if (focused_hwnd == bd->hWnd)
            {
                ...
                io.AddMousePosEvent((float)mouse_client_pos.x, (float)mouse_client_pos.y);
            }
        }
    }
}
...
void ImGui_ImplWin32_InitPlatformInterface()
{
	...
	main_viewport->PlatformHandle = (void*)vd->Hwnd;
}
