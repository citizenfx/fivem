...
{
	uint32_t unkt1; // 0
	uint32_t albedoHash; // 4
	uint32_t normalHash; // 8
	uint32_t materialHash; // 12
	float field16; // 16
	float field20; // 20
	float field24; // 24
	int field28; // 28
...
{
	uint16_t m_outfitIndex;
	uint16_t m_subOutfitIndex;
	uint16_t m_variationIndex;
	char m_outfitType;  
	char pad_7[1];
};

struct PedAppearanceDataNode
{
	MetaPedOutfitHandle m_outfit;

	AppearanceComponent components[32];

	unsigned __int64 array22792[32]; // 32 unsigned 64-bit integers
	char pad22792[23176 - 22792 - 32 * 8]; // Padding to 23176

	unsigned int array23176[32]; // 32 unsigned integers (4 bytes each)
	char pad23176[23560 - 23176 - 32 * 4]; // Padding to 23560

	float field23560; // 4-byte float
	unsigned int field23564; // 4-byte unsigned integer	
	unsigned char eMetaPedType; // 1-byte unsigned char
	uint8_t componentsCount;
	char field23570; // 1-byte char (bool) // most of cases 7
	char array23571[32]; // 32-byte array of bools/chars
	char field23603; // 1-byte char (bool)
	char field23604; // 1-byte char (player index)
	char field23605; // 1-byte char (bool)
	char field23606; // 1-byte char (flag)
	char pad[9];
	uint32_t modelHash;
	uint16_t objectId;
};
...
static hook::cdecl_stub<uint32_t(uint32_t* a1, uint32_t* a2, uint32_t* a3, uint8_t textureOwnerId, AppearanceComponent* appearance, uint8_t* textureIndex)> g_orgAcquireTexture([]()
{
	return hook::get_pattern("4C 8B DC 49 89 5B ? 49 89 73 ? 49 89 7B ? 4D 89 63 ? 55 41 56 41 57 48 8B EC 48 83 EC ? 48 8B 7D");
}); 
...
static bool searchForTexture(uint8_t textureOwnerId, uint8_t componentId, AppearanceComponent* appearance, uint32_t* textureId)
{
	...
	for (int i = 0; i < textureOverrideArraySize; i++)
	{
		...
		if (entry.m_textureOverrideOwnerId == textureOwnerId && entry.componentId == componentId)
		{
			...
			{
				// TODO: Check if found identical copy, then set textureId and return 0.
				//*textureId = i | (entry.m_textureOverrideReuseCount << 16);
				//return 0;
			}

			// For now, always clear if found.
			*textureId = i | (entry.m_textureOverrideReuseCount << 16);
			return 1;
		}
	}
