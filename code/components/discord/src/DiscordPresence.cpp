...
static void OnRichPresenceStateChange(internal::ConsoleVariableEntry<std::string>* richPresenceState)
{
	...
	if (state != "disabled" && state != "partial" && state != "enabled")
	{
	...
	if (g_lastRichPresenceState == "disabled" || (state == "partial" && g_lastDiscordAppId != DEFAULT_APP_ID))
	{
		if (state == "partial" && g_lastDiscordAppId != DEFAULT_APP_ID)
		{
