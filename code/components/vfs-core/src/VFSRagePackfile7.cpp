...
namespace vfs
{
	...
	bool RagePackfile7::OpenArchive(const std::string& archivePath)
	{
		...
		if (m_header.magic != 0x52504637 || m_header.encryption != 0x4E45504F) // OPEN, yes.
		{
	...
	bool RagePackfile7::FindNext(THandle handle, FindData* findData)
	{
		...
		if (handleData)
		{
			...
			return ((handleData->curOffset + 1) < handleData->entry.physFlags);
		}
