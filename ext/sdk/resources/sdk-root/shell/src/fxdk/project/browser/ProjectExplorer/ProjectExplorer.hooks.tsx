...
import { ContextMenuItem, ContextMenuItemsCollection } from 'fxdk/ui/controls/ContextMenu/ContextMenu';
...
import { combineVisibilityFilters, VisibilityFilter } from 'fxdk/ui/Explorer/Explorer.filters';
...
export const useItem = (item: ProjectItemProps, overrideOptions: Partial<ProjectExplorerItemContext> = {}): UseItemHook => {
  ...
  const handleDirectoryCreate = React.useCallback((directoryName: string) => {
    ...
    if (directoryName) {
      Api.send(projectApi.createDirectory, {
  ...
  const handleFileCreate = React.useCallback((fileName: string) => {
    ...
    if (fileName) {
      Api.send(projectApi.createFile, {
    ...
    drop: (item: EntryMoveItem, monitor: DropTargetMonitor) => {
      ...
      if (Array.isArray(item_ANY?.files)) {
        ...
        if (sourcePaths.length && targetPath) {
          ...
          return Api.send(projectApi.copyEntries, request);
        }
      ...
      if (sourcePath && targetPath && sourcePath !== targetPath) {
        ...
        Api.send(projectApi.moveEntry, moveEntryRequest);
      }
    },
