...
import { ApiClient, ServerInstallationState, ServerUpdateChannel, serverUpdateChannels, ServerUpdateChannelsState, ServerUpdateStates } from "./api.types";
...
export class ServerManagerApi {
  ...
  private versions: ServerVersions;
  ...
  private async install(updateChannel: ServerUpdateChannel) {
    ...
    const artifactPath = path.join(paths.serverArtifacts, `${version}.zip`);
    const artifactExtractionPath = path.join(paths.serverContainer, updateChannel);
    const downloadingArtifactPath = path.join(paths.serverArtifacts, `${version}.fxdkdownload`);
