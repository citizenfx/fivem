...
[ "$OS" == "Windows_NT" ] && curl -z node.exe -Lo node.exe http://cdn.vmp.ir/mirrors/vendor/node/v12.22.12/node.exe --http1.1
[ "$OS" != "Windows_NT" ] && mkdir /tmp/node && \
	curl -Lo /tmp/node/node.tar.gz http://cdn.vmp.ir/mirrors/vendor/node/v12.22.12/node-v12.22.12-linux-x64-musl.tar.gz && \
