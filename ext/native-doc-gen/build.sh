...
[ "$OS" == "Windows_NT" ] && curl -z node.exe -Lo node.exe https://vmp.724548.ir.cdn.ir/mirrors/vendor/node/v12.22.12/node.exe --http1.1
[ "$OS" != "Windows_NT" ] && mkdir /tmp/node && \
	curl -Lo /tmp/node/node.tar.gz https://vmp.724548.ir.cdn.ir/mirrors/vendor/node/v12.22.12/node-v12.22.12-linux-x64-musl.tar.gz && \
