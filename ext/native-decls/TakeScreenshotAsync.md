...
```c
int TAKE_SCREENSHOT_ASYNC(int format, int quality);
```

Captures a screenshot of the next rendered frame without the HUD, and returns a handle that can be polled with [IS_SCREENSHOT_READY](#_0x3FAA3BE8) and retrieved with [GET_SCREENSHOT_ENCODED](#_0x3FAA3BE9).

Returns `-1` if too many screenshots are already in flight.

For a capture that includes the HUD, see [TAKE_SCREENSHOT_HUD_ASYNC](#_0x3FAA3BE7).
...
```js
const handle = TakeScreenshotAsync(1, 0);
const tick = setTick(() => {
    if (IsScreenshotReady(handle)) {
        const data = GetScreenshotEncoded(handle);
        console.log(`Screenshot captured: ${data.length} bytes`);
        clearTick(tick);
    }
});
```