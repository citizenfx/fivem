...
export async function loadPlaytimes(serverId: string, cfxIds: TCFXID[]): Promise<Record<TCFXID, IServerActivityUserPlaytime>> {
  try {
    const searchParams = new URLSearchParams(cfxIds.map((cfxId) => ['identifiers[]', cfxId]));

    const response: Array<unknown> = await fetcher.json(`https://lambda.fivem.net/api/ticket/playtimes/${serverId}?${searchParams}`);
