...
export const ServerListItem = observer(function ServerListItem(props: ServerListItemProps) {
  ...
  const lclocaleCountry = server.localeCountry.toLowerCase();
  ...
  const showPremiumBadge = !hidePremiumBadge && !!server.premium;
  const showCountryFlag = !hideCountryFlag && lclocaleCountry !== 'aq' && lclocaleCountry !== '001';
    ...
    >
      ...
      {!hideActions && (
        <div className={clsx(s.actions, s['show-on-hover'])}>
          <Title fixedOn="bottom" title="BOOST™ Power">
          ...
          {!!burstPower && (
            <Title fixedOn="bottom" title="Burst Power">
              <Loaf bright size="small">
                <span>🔥</span>
                &nbsp;
                {burstPower}
