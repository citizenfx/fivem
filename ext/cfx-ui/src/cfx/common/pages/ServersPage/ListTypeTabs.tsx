...
export const serversListsEhers: Record<string, { title: React.ReactNode, icon: React.ReactNode, to: string, color: TextColor }> = {
  [ServersListType.All]: {
...
export const ListTypeTabs = observer(function ListTypeTabs() {
  ...
  const nodes = ServersService.listTypes.map((type) => {
    if (!serversListsEhers[type]) {
      return null;
    }

    const { title, icon, to } = serversListsEhers[type];
