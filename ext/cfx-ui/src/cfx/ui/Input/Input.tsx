...
// Date type
  | { type: 'date', min?: string, max?: string, value: string, onChange(value: string): void }
// Number is kinda a special snowflake
  | { type: 'number', min?: number | string, max?: number | string, value: number | string, onChange(value: string): void }

    // type TypeSpecific =
    // // For normal stringy types
    //   | { type?: 'text' | 'password' | 'search' | 'date', value: string, onChange(value: string): void }
    // // Number is kinda a special snowflake
    //   | { type: 'number', min?: number | string, max?: number | string, value: number | string, onChange(value: string): void }
...
export const Input = React.forwardRef(function Input(props: InputProps, ref: React.Ref<HTMLDivElement>) {
  ...
  const min = (props.type === 'number' || props.type === 'date') && (props.min) || undefined;
  const max = (props.type === 'number' || props.type === 'date') && (props.max) || undefined;
