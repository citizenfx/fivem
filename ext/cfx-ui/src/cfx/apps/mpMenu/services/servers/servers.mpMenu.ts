...
export class MpMenuServersService implements IServersService, AppContribution {
  ...
  private _promotedServerDescriptor: IPromotedServerDescriptor | null = null;
  public get promotedServerDescriptor(): IPromotedServerDescriptor | null { return this._promotedServerDescriptor }
  private set promotedServerDescriptor(promotedServerId: IPromotedServerDescriptor | null) { this._promotedServerDescriptor = promotedServerId }
  ...
  private async maybeLoadTopLocaleServer() {
    const promotedServerId = this.pinnedServersConfig?.noAdServerId;
    if (promotedServerId) {
      this.promotedServerDescriptor = {
        origin: 'ad',
        id: promotedServerId,
      };
