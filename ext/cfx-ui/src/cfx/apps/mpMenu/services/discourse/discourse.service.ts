...
class DiscourseService implements IAccountService, AppContribution {
  ...
  private _authToken: string = window.localStorage.getItem(LSKeys.DISCOURSE_AUTH_TOKEN) || '';
  ...
  init() {
    this.loadCurrentAccount().catch((error) => console.error(error)).finally(() => {
      this.accountLoadComplete = true;

      const authenticated = !!this.account;

      this.initialAuthCompleteDeferred.resolve(authenticated);
    });
  }
