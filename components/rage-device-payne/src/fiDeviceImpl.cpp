...
uint64_t WRAPPER fiDeviceImplemented::fileLengthLong(uint32_t file)
{
	PURECALL();
}

uint64_t WRAPPER fiDeviceImplemented::fileLengthByName(const char* fileName)
{
	PURECALL();
}
...
uint64_t WRAPPER fiDeviceImplemented::m_84(int a1)
{
	PURECALL();
}
}